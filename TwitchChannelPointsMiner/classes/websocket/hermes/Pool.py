...
class HermesWebSocketPool(WebSocketPool, HermesWebSocketListener):
    ...
    def __next_available_client(self) -> HermesClient:
        ...
        if client is None:
            client = self.__create_new_client(self.twitch.client_session.login, [])
            client.open()
        return client
    ...
    def __reconnect(self, client: HermesClient):
        ...
        with self.__lock:
            ...
            if self.clients[client.index].id != client.id:
                ...
                return
            new_client = self.__create_new_client(self.twitch.client_session.login, client.all_topics(), client.index)
